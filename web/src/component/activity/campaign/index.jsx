...
export default function New() {
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: "type",
            render: (_text, record) => {
                return record.type_obj.label;
            },
            width: 120,
        },
        {
            key: "registration_time",
            title: labels.registration_time,
            dataIndex: "registration_time",
            render: (_text, record) => {
                return (
                    <span>
                        Từ {moment(record.registration_from).format("DD/MM/yyyy")} đến {moment(record.registration_to).format("DD/MM/yyyy")}
                    </span>
                );
                // return _text;
            },
        ...
        {
            key: "occurring_time",
            title: labels.occurring_time,
            dataIndex: "occurring_time",
            render: (_text, record) => {
                return _text;
            },
            width: 100,
            ...
            render: (_text, record) => (
                <div className="flex-space">
                    <PemCheck pem_group={PEM_GROUP} pem="change">
                        <EditBtn onClick={() => navigateToAddCampaignPage(record.id)} />
                    </PemCheck>
    ...
    const navigateToAddCampaignPage = (id = 0) => {
        navigateTo("/activity/campaign/" + id);
    };
        ...
        return (
            <Space size={12}>
                ...
                <PemCheck pem_group={PEM_GROUP} pem="add">
                    <AddNewBtn onClick={() => navigateToAddCampaignPage()} />
                </PemCheck>
            </Space>
        );
    };
    return (
        <>
            {/* <Space></Space> */}
            <Card title={<Header />} extra={<SearchInput onChange={searchList} placeholder="Tìm kiếm tin tức" width="400px" />}>
                <CommonTable
                    columns={columns}
                    list={list}
                    loading={loading}
