...
from ..helper.sr import PositionSr


class PositionViewSet(GenericViewSet):
    _name = "position"
    serializer_class = PositionSr
    ...
    def list(self, request):
        ...
        serializer = PositionSr(queryset, many=True)
        return self.get_paginated_response(serializer.data)

    def retrieve(self, request, pk=None):
        obj = get_object_or_404(Position, pk=pk)
        serializer = PositionSr(obj)
        return RequestService.res(serializer.data)

    @action(methods=["post"], detail=True)
    def add(self, request):
        serializer = PositionSr(data=request.data)
    ...
    def change(self, request, pk=None):
        obj = get_object_or_404(Position, pk=pk)
        serializer = PositionSr(obj, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
